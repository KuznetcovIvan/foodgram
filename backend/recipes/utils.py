from io import BytesIO

from babel.dates import format_date
from reportlab.pdfbase.pdfmetrics import registerFont
from reportlab.pdfbase.ttfonts import TTFont
from reportlab.pdfgen.canvas import Canvas


def get_shopping_cart_pdf(product_list, date, request):
    """Генерирует PDF файл со списком покупок"""
    buffer = BytesIO()
    p = Canvas(buffer)
    registerFont(TTFont(
        'DejaVuSansMono', 'static/fonts/DejaVuSansMono-Oblique.ttf'))
    p.setFont('DejaVuSansMono', 14)
    date = format_date(date, format='d MMMM yyyy', locale='ru')
    x = 100
    y = 800
    p.drawString(x, y, f'Список покупок на {date} г.:')
    y -= 20
    p.drawString(x, y, '{}'.format('*' * 50))
    y -= 15
    for index, product in enumerate(product_list):
        if y < 50:
            p.showPage()
            p.setFont('DejaVuSansMono', 14)
            y = 800
        p.drawString(x, y,
                     '{index}. {ingredient} - {amount} {unit}.'.format(
                         index=index + 1,
                         ingredient=product['ingredient__name'].capitalize(),
                         amount=product['total_amount'],
                         unit=product['ingredient__measurement_unit']))
        y -= 20
    p.drawString(x, y, '*' * 50)
    y -= 10
    p.setFont('DejaVuSansMono', 10)
    p.drawString(300, y, 'Generated by {}'
                 .format(request.build_absolute_uri('/')[:-1]))
    p.showPage()
    p.save()
    buffer.seek(0)
    return buffer
